...
import * as Icon from '@/components/svgs/quote';
      ...
      {words.map((word, i) => {
        if (i === 0) {
          return (
            <span key={i} className="relative inline-flex">
              <Icon.Start className="absolute -top-5 -left-2 rounded-full text-accent bg-accent/10 shadow-[0_0_0_1.5rem_none] shadow-accent/10 lg:top-0 lg:-left-5 lg:-translate-x-2" />
              {word}&nbsp;
            </span>
          );
        }
        if (i === words.length - 1) {
          return (
            <span key={i} className="relative inline-flex">
              {word}
              <Icon.End className="absolute top-0 -right-5 translate-x-2 text-accent" />
            </span>
          );
        }
